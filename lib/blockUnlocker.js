	...
	async.waterfall([
		...
		function (blocks, callback) {
			async.filter(blocks, function (block, mapCback) {
				...
				let rpcMethod = config.blockUnlocker.enableStakingPhases ? 'f_block_json' : 'getblockheaderbyheight';
				let postRequest = config.blockUnlocker.enableStakingPhases ? { hash: block.hash } : { height: blockHeight };
				...
				apiInterfaces.rpcDaemon(rpcMethod, postRequest, function (error, result) {
					...
					let blockHeader;
					if(config.blockUnlocker.enableStakingPhases) {
						if (!result.block) {
					...
					if (config.blockUnlocker.enableStakingPhases) {
						block.reward = utils.getPhaseReward(blockHeader.height);
					}
