...
// Get phase reward function
exports.getPhaseReward = function (height) {
	let enableHeight = 86400;
	let halvingHeight = 1051200;
	let enableReward = 300000000;    // 3.00000000 VRM
	let halvingReward = 100000000;   // 1.00000000 VRM

  // If the height is greater than or equal to enableHeight, return the enable reward
  if (height >= enableHeight && height < halvingHeight) {
		return enableReward;
	}

	// If the height is greater than or equal to halvingHeight, start applying halving logic
	if (height >= halvingHeight) {
			// Determine how many halvings have occurred
			let halvings = Math.floor(height / halvingHeight);
			
			// Return the reward based on the halving logic
			return halvingReward / Math.pow(2, halvings - 1); // -1 because the first halving occurs at halvingHeight
	}

	// If the height doesn't meet any conditions, return a default value (optional)
	return enableReward;  // Or handle it based on your logic
}
