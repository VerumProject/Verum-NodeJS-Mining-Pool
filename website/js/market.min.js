(function ($) {
  ...
  $.marketInitTemplate = function(runOnce, chartsInitialized, loadedData, marketPrices, intervalChartsUpdate, currencyPairs, xhrMarketGets) {
      ...
      $(`#miningProfitCalc${coin}Network`).on('change', function() {
        $.marketCalcEstimateProfit(marketPrices);
      });
  ...
  $.marketCalcEstimateProfit = function(marketPrices) {
      ...
      try {
          ...
          let networkHashrate = (lastStats.network.difficulty / lastStats.config.coinDifficultyTarget);

          let addToNetwork = document.getElementById(`miningProfitCalc${coin}Network`).checked;
          if (addToNetwork) {
            networkHashrate = networkHashrate + hashRate;   
          }

          let blockADay = ((24 * 60) * 60) / lastStats.config.coinDifficultyTarget;
          let profit = (lastStats.lastblock.reward * blockADay) * (hashRate / networkHashrate);
          
          if (profit) {
              $.updateText(`calcHashAmount${coin}5Min`, $.getReadableCoin(lastStats, profit / 288));
              $.updateText(`calcHashAmount${coin}1Hour`, $.getReadableCoin(lastStats, profit / 24));
              $.updateText(`calcHashAmount${coin}12Hours`, $.getReadableCoin(lastStats, profit / 12));
              $.updateText(`calcHashAmount${coin}1Day`, $.getReadableCoin(lastStats, profit));

              /*$.updateText(`calcHashAmount${coin}5MinUSD`, $.marketGetCurrencyPriceText(lastStats, profit / 288, marketPrices));
              $.updateText(`calcHashAmount${coin}1HourUSD`, $.marketGetCurrencyPriceText(lastStats, profit / 288, marketPrices));
              $.updateText(`calcHashAmount${coin}12HoursUSD`, $.marketGetCurrencyPriceText(lastStats, profit / 288, marketPrices));
              $.updateText(`calcHashAmount${coin}1DayUSD`, $.marketGetCurrencyPriceText(lastStats, profit / 288, marketPrices));*/

              return;
          } else {
              $.updateText(`calcHashAmount${coin}5Min`, $.getReadableCoin(lastStats, 0));
              $.updateText(`calcHashAmount${coin}1Hour`, $.getReadableCoin(lastStats, 0));
              $.updateText(`calcHashAmount${coin}12Hours`, $.getReadableCoin(lastStats, 0));
              $.updateText(`calcHashAmount${coin}1Day`, $.getReadableCoin(lastStats, 0));

              /*$.updateText(`calcHashAmount${coin}5MinUSD`, $.getReadableCoin(lastStats, 0, marketPrices));
              $.updateText(`calcHashAmount${coin}1HourUSD`, $.getReadableCoin(lastStats, 0, marketPrices));
              $.updateText(`calcHashAmount${coin}12HoursUSD`, $.getReadableCoin(lastStats, 0, marketPrices));
              $.updateText(`calcHashAmount${coin}1DayUSD`, $.getReadableCoin(lastStats, 0, marketPrices));*/
          }
      } catch (e) {
        $.updateText(`calcHashAmount${coin}5Min`, $.getReadableCoin(lastStats, 0));
        $.updateText(`calcHashAmount${coin}1Hour`, $.getReadableCoin(lastStats, 0));
